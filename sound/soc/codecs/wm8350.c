...
static int wm8350_set_dai_sysclk(struct snd_soc_dai_runtime *codec_dai,
...
static int wm8350_set_clkdiv(struct snd_soc_dai_runtime *codec_dai, 
	int div_id, int div)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	u16 val;
...
static int wm8350_set_dai_fmt(struct snd_soc_dai_runtime *codec_dai,
		unsigned int fmt)
{
	struct snd_soc_codec *codec = codec_dai->codec;
...
static int wm8350_mute(struct snd_soc_dai_runtime *dai, int mute)
{
...
static int wm8350_set_fll(struct snd_soc_dai_runtime *codec_dai,
...
static int wm8350_set_tdm_slot(struct snd_soc_dai_runtime *codec_dai,
...
static int wm8350_set_tristate(struct snd_soc_dai_runtime *codec_dai, 
	int tristate)
{
	//struct snd_soc_codec *codec = codec_dai->parent.codec;
...
static int wm8350_codec_probe(struct device *dev)
{
	...
	ret = snd_soc_codec_add_dai(codec, &wm8350_dai, 1);
	if (ret < 0)
		goto err;
...
const char wm8350_codec_id[] = "wm8350";
EXPORT_SYMBOL_GPL(wm8350_codec_id);

static struct device_driver wm8350_codec_driver = {
...
{
	return driver_register(&wm8350_codec_driver);
}

static __exit void wm8350_exit(void)
{
	driver_unregister(&wm8350_codec_driver);
}
