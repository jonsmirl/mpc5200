...
static int wm8974_set_pll(struct snd_soc_dai *codec_dai,
...
static int wm8974_set_fmt(struct snd_soc_dai *codec_dai,
		unsigned int fmt)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	u16 iface = 0;
...
static int wm8974_hw_params(struct snd_pcm_substream *substream,
...
static int wm8974_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	struct snd_soc_card *soc_card)
{
	...
	wm8974_add_controls(codec, soc_card->card);
	wm8974_add_widgets(codec, soc_card);

	return 0;
}

static struct snd_soc_codec_ops wm8974_codec_ops = {
	.dapm_event	= wm8974_dapm_event,
...
static const struct snd_soc_pcm_stream wm8974_dai_playback = {
...
static const struct snd_soc_pcm_stream wm8974_dai_capture = {
	.stream_name	= "Capture",
	.channels_min	= 1,
	.channels_max	= 1,
	.rates		= WM8974_RATES,
...
static const struct snd_soc_dai_ops wm8974_dai_ops = {
	.digital_mute	= wm8974_digital_mute,
	.set_fmt	= wm8974_set_fmt,
	.set_pll	= wm8974_set_pll,
	.set_clkdiv	= wm8974_set_clkdiv,
};

/* audio ops, called by alsa */
static const struct snd_soc_ops wm8974_dai_audio_ops = {
	.hw_params	= wm8974_hw_params,
};

static int wm8974_dai_probe(struct device *dev)
{
	struct snd_soc_dai *dai = to_snd_soc_dai(dev);
	
	dai->ops = &wm8974_dai_ops;
	dai->audio_ops = &wm8974_dai_audio_ops;
	dai->capture = &wm8974_dai_capture;
	dai->playback = &wm8974_dai_playback;
	return 0;
}
